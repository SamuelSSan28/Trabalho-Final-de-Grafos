...
param  = sys.argv[1:] #recebe inicio e final param[0]: inicio
                                           # param[1]: destino
if param[0].lower() == 'rua' or param[0].lower() == 'alameda' or param[0].lower() =='praça' or param[0].lower() == 'avenida':
    x = ''
    aux = []
    for i in param:
        if i =="*":
            aux.append(x[0:len(x)-1])
            x = ""
        else:
            x += i + " "
    aux.append(x[0:len(x)-1])

    dados_rua = open("C:\\Users\\samue\\PycharmProjects\\graph\\dicio.txt") #leitura dos dados do grafo

    ruas = {}
    for j in dados_rua: #leitura do arquivo do dicionario
        j = j.replace('\n', '')
        j = j.split(';')
        print(j)
        ruas.update({j[0]: j[1]})
    ...
    param = []
    for i in ruas:
        #print(len(aux[0].lower()),len(ruas[i].lower()))
        if ruas[i].lower() == aux[0].lower():
            param.append(i)
            break

    for i in ruas:
        if ruas[i].lower() == aux[1].lower():
            param.append(i)
            break
...
def main():
    dados = open("C:\\Users\\samue\\PycharmProjects\\graph\\obj3.txt") #leitura dos dados do grafo
